...
| `activations.c/h` | Stores activation functions (ReLU, tanh, softmax). |
...
- **Softmax:**  
  ...
  - Uses exponential approximation via lookup table.  

- **Others (future):**  
  - Tanh, Sigmoid, or custom physics-informed activations using the same table-driven approach.
...
- Contains:
  ...
  - Exponential approximation with lookup table + interpolation.  
  - Optional compile-time switches for float or fixed-point modes.
...
- Results are normalized to integer range (0–255 for `uint8_t`) and written to `exp_lut.h`.  
...
- Table values:
  ...
  - Rise exponentially toward 255 for large positives.
...
2. Compute fractional index between nearest samples.  
3. **Linearly interpolate** between two LUT values for smooth output.  
4. Return approximated exponential (float or integer scaled).
...
- Regenerate only when:
  ...
  - Output format changes (e.g., move to `uint16_t` or fixed Q format).  
...
- ✅ Exponential lookup table + interpolation integrated.  
...
1. Implement integer-only (Q7/Q15) exponential interpolation.  
2. Add tanh and sigmoid LUTs using the same approach.  
3. Validate on Arduino Nano 33 BLE Sense with test vectors.  
